...
export function Redirect(location, code = 302, extras = {}) {
  ...
  if ("string" !== typeof location) {
    throw new TypeError(
      "A redirection `location` string or object must be given"
    );
  }
  if (((code / 100) | 0) !== 3) {
    throw new TypeError(
      `\`code\` must be a 3xx redirect status code (i.e. 302), got ${code}`
    );
  }
...
export function UnauthorizedError(message, extras = {}) {
  ...
  if (bearer) {
    if (!extras.headers) extras.headers = {};
    extras.headers["WWW-Authenticate"] =
      "Bearer " +
      Object.keys(bearer)
    ...
    throw new TypeError(
      'A `bearer` object MUST be given to create the "WWW-Authenticate" HTTP header'
    );
  }
